...
from telegram import BotCommand, InlineKeyboardMarkup, MenuButtonCommands, Update, User
...
async def start_task(update: Update, context: CallbackContext):
    ...
    try:
        ...
        def l(s):  # noqa: E743
            return context.application.base_app.localization(
                s, locale=update.effective_user.language_code
            )
        ...
        if context.application.base_app.users_collection is not None:
            try:
                ...
                logger.error(
                    f"mongodb update error {update.effective_user=}: {e}", exc_info=1
                )
...
class TGUpdate(TGState):
    ...
    async def state_waiting_text(self):
        ...
        if (filters.TEXT & ~filters.COMMAND).check_update(self.update):
            ...
            logger.debug(
                f"was waiting text input from user {self.user}, got something else, falling back"
            )
    ...
    async def state_empty(self):
        ...
        for _, plugin in self.context.application.plugins.items():
            priority, handle = plugin.test_message(
                self.update, self.state, self.web_app_data
            )
        ...
        if chosen_plugin is not None:
            logger.info(
                f"from {hits} accepting plugins for user {self.user} selected plugin {chosen_plugin.name} based on priority {chosen_priority}"
            )
            ...
            await self.update.message.reply_markdown(
    ...
    def parse_web_app_data(self):
        try:
            self.web_app_data = json.loads(
                self.update.effective_message.web_app_data.data
            )
        except Exception as e:
    ...
    async def parse(self):
        try:
            ...
            await self.reply(
                self.l("something-went-wrong"), parse_mode=ParseMode.MARKDOWN
            )
    ...
    async def parse_callback_query(self):
        try:
            ...
            logger.error(
                f"Failed to parse callback query {self.user=}: {e}", exc_info=1
            )
                ...
                result = await user_db.update_one(
                    {
                        ...
                        "state.deadline": {
                            "$lte": user["state"]["deadline"]
                            + datetime.timedelta(seconds=1),
                            "$gte": user["state"]["deadline"]
                            - datetime.timedelta(seconds=1),
                        },
                    },
                    {"$set": {"state": {"state": ""}}},
    ...
    try:
        ...
        if user is None:
            return await update.reply(
                update.l("user-is-none"), parse_mode=ParseMode.HTML
            )
        if "banned" in user:
            return await update.reply(
                update.l("user-is-restricted"), parse_mode=ParseMode.HTML
            )
        await update.parse()
    except Exception as e:
...
async def parse_callback_query_task(tgupdate: Update, context: CallbackContext):
    try:
        ...
        user = await update.get_user()
        if "banned" in user:
            return await update.edit_or_reply(
                update.l("user-is-restricted"), parse_mode=ParseMode.HTML
            )
        await update.parse_callback_query()
    except Exception as e:
        logger.error(
            f"Exception in parse_callback_query_task: {e}, {tgupdate=}", exc_info=1
        )
...
async def create_telegram_bot(config: Config, app, plugins) -> TGApplication: # type: ignore
    ...
    application = (
        ...
        .token(token=config.telegram.token.get_secret_value())
        .build()
    )
    ...
    application.add_handler(
        MessageHandler(
            filters.StatusUpdate.WEB_APP_DATA & filters.ChatType.PRIVATE, parse_message
        )
    )
