...
class Avatar(BasePlugin):
    ...
    def swap_faces(
        ...
        _, _, restored = refiner_model.enhance(
    ...
    async def handle_image_stage2(self, update: TGState, name):
        ...
        if user_role == "":
            return await update.reply(
                update.l("avatar-no-role"), parse_mode=ParseMode.HTML
            )
        ...
        with Image.open(file_path) as img:
            try:
                ...
                return await update.reply(
                    update.l("avatar-no-face"), parse_mode=ParseMode.HTML
                )
            ...
            with Image.open(files["face_file"]) as face:
                ...
                logger.info(
                    f"face swap for {update.user} took: {time.time() - swap_start} seconds"
                )
                ...
                with Image.open(files["frame_file"]) as frame:
                    with Image.open(files["mask_file"]) as mask:
                        ...
                        final.save(
                            final_name, quality=self.config.photo.quality, optimize=True
                        )
