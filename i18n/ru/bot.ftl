...
passes-adm-payment-proof-accepted =
    Оплата {$type ->
        [couple] парногo пасса на Zouk Non Stop для {passes-role} по имени <i>{$name}</i> и второго участника {$coupleLink} (<i>{$coupleName}</i>)
        *[solo] пасса на Zouk Non Stop для {passes-role} по имени <i>{$name}</i>
    } подтверждена.
...
passes-adm-payment-proof-received =
    Пользователь {$link} отправил оплату за пасс 
    {$type ->
        [couple] для {passes-role} по имени <i>{$name}</i> и второго участника {$coupleLink} (<i>{$coupleName}</i>)
        *[solo] для {passes-role} по имени <i>{$name}</i>
    }.
...
passes-added-to-waitlist =
    ...
    Тем не менее, вы добавлены в очередь на {$type ->
        [couple] парный пасс на Zouk Non Stop с вашей ролью {passes-role} и вторым участником {$coupleLink}
        *[solo] пасс {passes-role} на Zouk Non Stop
    }. Ожидайте дальнейшего уведомления.
...
passes-pass-edit-waitlist=
    Привет, <i>{$name}</i>!
    Вы находитесь в списке ожидания для {$type ->
        [couple] парного пасса на Zouk Non Stop с вашей ролью {passes-role} и вторым участником {$coupleLink}
        *[solo] {passes-role} пасса на Zouk Non Stop
    }.
...
passes-pass-assigned =
    ...
    Поздравляем! {$type ->
        [couple] Парный пасс на Zouk Non Stop с вашей ролью {passes-role} и вторым участником {$coupleLink}
        *[solo] Пасс для {passes-role} на Zouk Non Stop
    } только что был назначен вам.

    Этот пасс пока не является официально вашим. Вам необходимо провести оплату в течение 7 дней с момента назначения.
    Текущая цена: <code>{$price}</code> ₽. Оплату можно произвести через СБП, используя номер телефона <code>{$phoneSBP}</code> ({$banksRu}), получатель {$adminLink}.
...
passes-pass-edit-waiting-for-couple=
    Привет, <i>{$name}</i>!
    Вы выбрали парный пасс в роли {passes-role} на Zouk Non Stop.
...
passes-pass-edit-payed=
    ...
    Поздравляем! {$type ->
        [couple] Парный пасс на Zouk Non Stop с вашей ролью {passes-role} и вторым участником {$coupleLink}
        *[solo] Пасс для {passes-role} на Zouk Non Stop
    } теперь официально ваш, так как вы успешно оплатили его.
...
passes-payment-request-callback-message =
    ...
    Оплата может быть произведена через СБП, используя номер телефона <code>{$phoneSBP}</code> ({$banksRu}), получатель {$adminLink}.
...
passes-payment-proof-accepted =
    ...
    Поздравляем! {$type ->
        [couple] Парный пасс на Zouk Non Stop с вашей ролью {passes-role} и вторым участником {$coupleLink}
        *[solo] Пасс для {passes-role} на Zouk Non Stop
    } теперь официально ваш, так как ваше подтверждение оплаты было успешно подтверждено администратором.
...
passes-pass-role-saved =
    Вы выбрали пасс {passes-role}.
    
    Вы берёте соло пасс или парный пасс?
passes-pass-cancelled-by-other =
    Ваша пара только что отменила парный пасс. К сожалению, вам придётся зарегистрироваться заново используя команду /passes.
...
passes-couple-request-wrong-data =
    Я ожидала от вас получить пересланное сообщение от вашей пары, а это не выглядит как пересланное сообщение. Чтобы попробовать ещё раз, введите команду /passes снова.
...
passes-couple-saved =
    ...
    Попросите вашу пару начать разговор со мной, прислав мне команду /start, а затем принять приглашение, отправив команду /passes.
    Приглашение действительно в течение двух дней.
passes-couple-invitation =
    Пользователь <i>{$coupleName}</i>, {$coupleLink} пригласил вас на марафон Зук Нон Стоп.
...
passes-couple-saved-sent =
    ...
    Попросите вашу пару подтвердить его. Если приглашение не пришло, ваша пара может найти его по команде /passes.
    Приглашение действительно в течение двух дней.
