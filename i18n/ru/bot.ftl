...
avatar-simple-caption = Добро пожаловать в картинную галерею нашего Нотр-Дама
...
orders-grodno-overview-full-error=К сожалению, все 20 мест на обзорную экскурсию уже заняты. Выбери «Городницу» или убери экскурсию по Гродно из заказа.
orders-grodno-gorodnitsa-full-error=К сожалению, все 25 мест на экскурсию «Городница» уже заняты. Выбери обзорную экскурсию или убери экскурсию по Гродно из заказа.
orders-capacity-service-shuttle = трансфер Минск — Гродно
orders-capacity-service-grodno-overview = обзорную экскурсию по Гродно
orders-capacity-service-grodno-gorodnitsa = экскурсию «Городница»
orders-payment-reminder =
    Заказ для <i>{$name}</i> на сумму <b>{$total} BYN</b> пока не оплачен.
    Места на трансфер и экскурсии закрепляются после отправки подтверждения оплаты. Если отложить оплату, свободные места могут закончиться и заказ будет пересчитан.
orders-capacity-unpaid-removed =
    Подтверждение оплаты не было отправлено вовремя, а места на {$services} закончились.
    Мы убрали недоступную услугу из заказа. Новая сумма заказа: <b>{$total} BYN</b>.
orders-capacity-proof-too-late =
    К моменту получения подтверждения оплаты места на {$services} уже закончились.
    Мы убрали недоступную услугу из заказа. Новая сумма заказа: <b>{$total} BYN</b>.
...
food-payment-method =
    {$iban ->
        ...
        [private] Амбассадор {$adminLink} принимает переводы на IBAN. Напишите ему, чтобы получить реквизиты.
       *[iban] Оплату можно произвести переводом на IBAN <code>{$iban}</code>, контакт {$adminLink}.
    }
...
passes-payment-admin-desc=
    {$adminEmoji} {$adminLink} {$iban ->
        ...
        [private] — оплата переводом на IBAN; напишите за реквизитами.
       *[iban] — оплата переводом на IBAN.
    }
